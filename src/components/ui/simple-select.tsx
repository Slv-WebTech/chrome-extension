...
export function SelectContent({ children }: SelectContentProps) {
  ...
  return (
    <div className="absolute top-full left-0 z-50 w-full mt-1 rounded-md border border-gray-300 bg-white shadow-lg">
      <div className="p-1">
        {children}
      </div>
    </div>
...
export function SelectItem({ value, className = '', children, ...props }: SelectItemProps) {
  ...
  return (
    <div
      className={`relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-gray-100 focus:bg-gray-100 ${selectedValue === value ? 'bg-blue-50 text-blue-600' : ''} ${className}`}
      onClick={handleClick}
      {...props}
    >
      {selectedValue === value && (
